...
function navButtonClass(active: boolean) {
  return active
    ...
    : "text-slate-600 hover:text-emerald-900 hover:bg-white/70 border-transparent";
}
...
function DropdownNav({
  ...
  return (
    <div className="group relative">
      ...
      <div className="pointer-events-none invisible absolute left-0 top-full mt-2 w-96 -translate-y-1 rounded-xl border border-slate-200 bg-white p-2 opacity-0 shadow-xl transition-all group-hover:pointer-events-auto group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
        {items.map((item) => (
          <button
            key={item.path}
            onClick={() => onNavigate(item.path)}
            className={`flex w-full items-start gap-3 rounded-lg p-3 text-left transition-colors ${isPathActive(currentPath, item.path) ? "bg-emerald-50 text-emerald-900" : "hover:bg-slate-50 text-slate-700"}`}
          >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-50 to-emerald-50/30">
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-8">
            ...
            >
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-gradient-to-br from-emerald-700 to-emerald-500">
          ...
          <div className="flex items-center gap-2">
            <button className="relative rounded-lg p-2 text-slate-600 transition-colors hover:bg-white hover:text-emerald-900">
              <Bell className="h-4 w-4" />
              <span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-rose-500" />
            </button>
            <button className="rounded-lg p-2 text-slate-600 transition-colors hover:bg-white hover:text-emerald-900">
      ...
      <main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-4 shadow-sm backdrop-blur-sm sm:p-6 lg:p-8 animate-fade-in">
